...
"""Run the module as a service: ``python -m openbdk_algorand``."""
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="openbdk-algorand", description="Blockscout-shaped read API for Algorand")
    ...
    try:
        ...
        raise SystemExit("the service extra is required: pip install 'openbdk-algorand[service]'") from exc

    uvicorn.run(
        "openbdk_algorand.service:app",
        host=args.host,
        port=args.port,
